...
const RELEASE_NOTES: ReleaseNote[] = [
  {
    ...
    date: "14 Aug 2026",
    summary:
      "Withdraws the fan check on NVIDIA cards, which 0.7.1 should not have claimed. The documented interface reports the fan speed the driver intends rather than the speed measured at the fan, and states that a physically blocked fan will not change that number, so the reading cannot show the failure it was there to find. The first card to run 0.7.1 reported 0 RPM for five minutes at 220 W with its fans plainly turning. There is a separate interface that does read a real tachometer, but it covers one fan of the several most cards have, and the one that covers them all is not published, so neither can say whether the cooler as a whole is working. No certificate was affected, because a reading of zero never met the threshold to fail anything. The check stands on AMD, where the fan speed is measured rather than intended.",
    changesResults: true,
  },
